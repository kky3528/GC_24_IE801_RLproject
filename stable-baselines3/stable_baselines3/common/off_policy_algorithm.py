...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        ### guided-exploration implementation
        self.using_guided_exploration = True
    ...
    def explore(self, state):
        ...
        for action in range(self.action_space.n):
            ...
            next_state = self.dynamics_model(np.append(state, [[action]], axis=1), training=False)
    ...
    def init_dynamics_model(self):
        model = Sequential()
        state_shape = (self.observation_space.shape[0] + 1,)
        print(state_shape)
    ...
    def fit_dynamics_model(self):
        batchsize = 64
        if self.replay_buffer.size() < batchsize: return
        samples = self.replay_buffer.sample(batchsize, env=self._vec_normalize_env)
        sampled_states = []
        sampled_targets = []
        for i in range(batchsize):
            state = samples.observations[i].cpu().numpy()
            action = samples.actions[i].cpu().numpy()
            reward = samples.rewards[i]
            new_state = samples.next_observations[i].cpu().numpy()
            done = samples.dones[i]
            input_state = np.append([state], [action], axis=1)
            target = np.array([new_state])
    ...
    def eval_dynamics_model(self):
        samples = self.replay_buffer.sample(32, env=self._vec_normalize_env)
        sampled_states = []
        sampled_targets = []
        for i in range(32):
            state = samples.observations[i].cpu().numpy()
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            if self.using_guided_exploration:
                self.update_count += 1
                if self.update_count % 25 == 0: self.fit_dynamics_model()
                if self.update_count % 500 == 0: self.eval_dynamics_model()
            ### guided-exploration implementation
